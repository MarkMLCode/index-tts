...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Deprecated compatibility option; IndexTTS 2.5 uses --model-dir.",
    )
    ...
    parser.add_argument(
        ...
        help="Model config YAML (default: checkpoints/config.yaml).",
    )
    parser.add_argument(
        "--gpt-checkpoint",
        type=Path,
    ...
    parser.add_argument("--language", type=str, default="en", help="Fallback language.")
...
def launch_worker(
    ...
    cmd = [
        ...
        str(args.val_ratio),
        "--audio-root",
        str(args.manifest.expanduser().resolve().parent),
    ]
    if args.tokenizer is not None:
        cmd.extend(["--tokenizer", str(args.tokenizer)])
...
def main() -> None:
    ...
    hf_cache_dir = (
        ...
        else (args.model_dir.expanduser().resolve() / "hf_cache")
    )
    ...
    if cache_has_required_assets(hf_cache_dir):
        ...
        print("[preprocess_multiproc] 2.5 auxiliary cache is incomplete; model setup may download missing assets.")
