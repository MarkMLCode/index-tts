...
def build_pairs(
    ...
    prompt_grouped: Optional[Dict[str, List[Sample]]] = None,
) -> List[Dict]:
    output: List[Dict] = []
    for speaker, items in grouped.items():
        if len(items) < 2 and prompt_grouped is None:
        ...
        for target in items:
            ...
            prompt_items = (prompt_grouped or grouped).get(speaker, items)
            prompts = [sample for sample in prompt_items if sample.id != target.id]
            if not prompts:
                continue
            ...
            for prompt in chosen:
                pair_record = {
                    ...
                    "prompt_language": prompt.record.get("language", ""),
                    "target_id": target.id,
                    ...
                    "target_language": target.record.get("language", ""),
                    "feature_version": target.record.get("feature_version", "2.0"),
                }
